...
interface SeriesContentCreditsCarouselProps {
  credits: SeriesAggregateCreditsCast | SeriesAggregateCreditsCrew | undefined;
}

const SeriesContentCreditsCarousel = ({ credits }: SeriesContentCreditsCarouselProps) => {
    ...
    >
      {credits?.map((credit, index) => (
        <Link key={index} href={`/person/${credit.id}`}>
          <div className={styles["series-content__credits-slide"]}>
            {credit?.profile_path ? (
              <Image
            ...
            <p
              className={styles["series-content__credits-slide-title"]}
              style={{ maxWidth: imageWidth }}
            >
              {credit.name}
            </p>
            {"jobs" in credit && credit.jobs && (
              <p className={styles["series-content__credits-slide-job"]}>
                {credit.jobs?.map((job) => job.job).join(", ")}
              </p>
            )}
            {"roles" in credit && credit.roles && (
              <p className={styles["series-content__credits-slide-character"]}>
...
export default SeriesContentCreditsCarousel;
