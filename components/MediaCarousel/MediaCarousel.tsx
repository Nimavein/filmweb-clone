...
const MediaCarousel = ({ medias, mediaType, title, ...props }: MediaCarouselProps) => {
  ...
  return (
    medias && (
      <div>
        {title && <h2 className={styles[`${baseCSSClassName}__title`]}>{title}</h2>}
        <Carousel {...props} className={styles[baseCSSClassName]}>
          {medias?.results?.map((media, index) => (
            <Link key={index} href={getMediaHref(mediaType, media.id)}>
                ...
                >
                  {`${getName(media)} (${
                    "release_date" in media
                      ...
                      : "first_air_date" in media && media.first_air_date?.slice(0, 4)
                  })`}
                </p>
              </div>
            </Link>
